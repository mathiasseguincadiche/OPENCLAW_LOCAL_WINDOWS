Describe 'Ollama OpenClaw compatibility supervisor' {
    ...
    It 'locks the compatibility proxy to loopback and the exact Ministral model' {
        $script:Supervisor | Should -Match "\$TaskName = 'OPENCLAW_LOCAL Ollama Compat'"
        $script:Supervisor | Should -Match '\$ListenPort = 11436'
        $script:Supervisor | Should -Match "\$ExpectedUpstream = 'http://127\.0\.0\.1:11434'"
    ...
    It 'installs a managed Task Scheduler wrapper and verified proxy copy' {
        $script:Supervisor | Should -Match "Register-ScheduledTask -TaskName \$TaskName"
    ...
    It 'starts the compatibility proxy before OpenClaw configuration in install-full' {
        ...
        $InstallIndex = $script:Installer.IndexOf("Action = 'install'", [System.StringComparison]::Ordinal)
        $StartIndex = $script:Installer.IndexOf("Action = 'start'", [System.StringComparison]::Ordinal)
        $ConfigureIndex = $script:Installer.IndexOf("Invoke-OpenClawConfigWriteWindow", [System.StringComparison]::Ordinal)
    ...
    It 'requires compatibility health before nominal ollama-vulkan admission' {
        ...
        $script:Configurator | Should -Match '(?s)if \(\$BackendId -eq ''ollama-vulkan''\).*Test-OllamaReady.*Test-OllamaCompatReady'
    }
}
